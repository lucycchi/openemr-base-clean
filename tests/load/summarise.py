...
tables used in clinical_copilot/BASELINES.md.
    ...
    with path.open() as fh:
        for row in csv.DictReader(fh):
            try:
                ...
                continue
            name = row["container"]
            kind = "db" if ("mysql" in name or "mariadb" in name) else "app"
...
def main() -> None:
    ...
    print("| VUs | Scenario | Requests | req/s | Briefs (cache hit %) | Asks | Model calls | Brief p50/p95/p99 (ms) | Cache-hit brief p50/p95 | Cold brief p50/p95 | Ask p50/p95/p99 (ms) | Chart open p50/p95 | HTTP errors | Co-Pilot errors | Summary unavailable | Verification fail |")
    print("|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|")
    for r in runs:
        L = r["latency_ms"]
        b, bh, bc, a, c = L["brief"], L["brief_cache_hit"], L["brief_cold"], L["ask"], L["chart_open"]
        print(
            f"| {r['vus']} | {r['scenario']} | {r['requests_total']} | {r['throughput_rps']} | {r['briefs']} ({n(r['brief_cache_hit_pct'])}) | {r['asks']} | {r['model_calls']} "
    ...
    print("| VUs | Scenario | App CPU avg / peak (% of one core) | App memory avg / peak (MiB) | DB CPU avg / peak | DB memory avg / peak (MiB) | Host load1 peak |")
    print("|---|---|---|---|---|---|---|")
    for r in runs:
        s = r["stats"]
        app, db = s.get("app", {}), s.get("db", {})
        print(
