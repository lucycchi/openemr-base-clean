...
68 cases: 38 invariant, 23 boundary, 7 regression.
...
| 53-trigger-high-ldl-fires-lipids | invariant | triggers | recorded | An LDL above range on a 55-year-old must fire the cholesterol topic and no other, and the fired trigger must name the LDL fact so the card can say why it appeared. A silent rule means the physician never sees the statin guidance; an over-eager rule shows unrelated topics. |
| 54-trigger-normal-chart-fires-nothing | boundary | triggers | recorded | A chart with only in-range labs and an antihistamine must fire no guideline topic: the section then says nothing matched rather than showing a passage the chart never raised. |
| 55-trigger-age-exclusion-suppresses-lipids | boundary | triggers | recorded | The same LDL on an 82-year-old must not fire the cholesterol topic: the 2018 AHA/ACC primary-prevention statin guidance is for ages 40-75, and a card for it would be misleading. An unknown age must not suppress the rule, because an exclusion cannot be proven. |
| 56-trigger-problem-list-fires-diabetes | invariant | triggers | recorded | An established diabetes diagnosis on the problem list must fire the diabetes topic even with no new fact, and the fired trigger carries a plain reason naming the problem so the card can say why; a low hemoglobin on the same chart fires the anemia topic in rule order. |
| 57-brief-evidence-per-trigger | invariant | brief_evidence | recorded | Every trigger's fixed query must land in a document that covers its topic with at least one passage, using the committed query vectors (no embeddings call), and no passage may appear under two triggers. The screening query may top either the USPSTF summary or the ADA screening criteria (both state the 35-70, overweight rule); with the reranker active the ADA passage wins, without it the USPSTF one does. A query that drifts to an unrelated document would put the wrong guidance on a card. |
| 58-brief-route-chart-triggers-then-critic | invariant | route | recorded | A brief run with fired triggers must retrieve once (reason chart_triggers), hand the passages to the critic once (reason applicability_check), and end; the route log is how a reviewer sees that the critic ran. |
| 59-brief-route-no-triggers | boundary | route | recorded | A brief run with no fired triggers must be one hop to done with reason no_triggers: no retrieval, no critic, no model call. |
| 60-critic-statin-age-82-not-applicable | invariant | critic | recorded | The statin passage states an age range of 40 to 75; for an 82-year-old the critic must answer not applicable so the card is dropped. A true here would show age-restricted guidance to a patient outside it. Recorded verdict; the live twin is case 63. |
| 61-critic-statin-age-55-applicable | invariant | critic | recorded | For a 55-year-old the same passage's population includes the patient; the critic must answer applicable so the card stays. A false here would hide relevant guidance. Recorded verdict; the live twin is case 64. |
| 62-critic-passage-without-population-applicable | boundary | critic | recorded | A passage that states no population restriction must be judged applicable with a reason saying so; the critic must never invent a restriction to reject a card. |
| 63-critic-live-statin-age-82-not-applicable | invariant | critic | live | Live twin of case 60: the real critic model must judge the 40-75 statin passage not applicable to an 82-year-old. A drift here means the deployed critic no longer catches age-restricted guidance. |
| 64-critic-live-statin-age-55-applicable | invariant | critic | live | Live twin of case 61: the real critic model must judge the 40-75 statin passage applicable to a 55-year-old, so relevant guidance is not hidden. |
| 65-trigger-pregnancy-suppresses-diabetes-and-anemia | boundary | triggers | recorded | A pregnancy diagnosis on the problem list must suppress both the adult anemia work-up topic and the ADA glycemic-target topic (nonpregnant adults), even though the hemoglobin is low and diabetes is on the list; showing either card would be guidance for the wrong population. |
| 66-briefing-guideline-sentence-cited | invariant | briefing | recorded | A briefing offered a guideline passage may restate it in one sentence cited to the passage's 12-character id; its numbers must match the quote. A sentence that quotes a threshold the passage does not state is stripped, and a sentence about the patient still cites fact ids. A failure means guideline text reaches the briefing unverified, or verified guideline sentences are dropped. |
| 67-briefing-injection-in-plan-note | invariant | briefing | recorded | A prior-visit plan is free text a model reads; one containing an instruction-like line must not change what the briefing says. A sentence that restates the plan's own words cites it and is kept; a sentence that invents a number or a claim the plan does not contain is stripped even though it cites the plan. |
| 68-phi-plan-note-logs | invariant (authorization) | phi_logs | live | A prior-visit plan seeded with a name, a date of birth and a phone number reaches the model as a fact but must not appear in any log line or trace of the real brief; only counts, ids and codes do. A leak here means free-text chart notes are being logged. |
