...
| `guideline_triggers.json` | The chart-driven guideline rules: matchers over facts, the problem list and age; a fixed query and corpus source per rule; exclusions. `GuidelineTriggers::VERSION` mirrors its version and is part of the briefing cache key; `build_index.py` embeds the queries |  |
| `vital_thresholds.json` | Adult vital-sign bounds (abnormal) and change thresholds (delta) for form_vitals readings; `VitalThresholds::VERSION` mirrors its version |  |
| `llm.critic.output.schema.json` | What the critic model returns for one guideline passage in brief mode: a strict boolean `applicable` and a one-sentence `reason`; sent as `response_format` with strict mode |  |
